...
namespace DATAExtractionUSINGADB
{
    public partial class Form2 : Form
    {
        ...
        string connstring = "userId=root;password=0622#I;server=localhost;database=at";
        ...
        {
            string output = RunADBCommand("shell content query --uri content://contacts/phones/ --projection display_name:number --limit 100");
               ...
               foreach (string line in output.Split('\n'))
                {
                    ...
                    {
                        InsertToDB($"INSERT INTO contactstable (Name, PhoneNumber, Email) VALUES ('{Escape(name,100)}', '{Escape(phone,20)}', '')");
                    }
                }
        ...
        {
            string output = RunADBCommand("shell content query --uri content://sms/ --projection address:date:body --limit 100");
            ...
            foreach(string line in output.Split('\n'))
            {
                ...
                {
                    InsertToDB($"INSERT INTO messagestable (Sender, Message, Timestamp) VALUES ('{Escape(senderAddr,100)}', '{Escape(body,50)}', FROM_UNIXTIME({date}/1000))");
                }
            }
        ...
        {
            string output = RunADBCommand("shell content query --uri content://call_log/calls/ --projection number:type:duration --limit 100");
            ...
            foreach (string line in output.Split('\n'))
            {
                ...
                {
                    InsertToDB($"INSERT INTO calllogstable (Number, CallType, Duration) VALUES ('{Escape(number,20)}', '{callType,20}', {duration ?? "0"})");
                }
            }
